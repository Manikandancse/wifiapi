<?php
	...
	use \Psr\Http\Message\ResponseInterface as Response;
		...
		$username      	= $args["customerId"];
		$password    	= $args["password"];
		$LoginTime     	= date("Y-m-d H:i:s");

		$sql = "SELECT * from  customers where `customer_id` = '".$username."' and 
										  `password` = '".$password."'";
										  
		try {
			$this->db->exec($sql);
			$data = array('message' => 'Login successfully');
	...
	$app->put('/edituser/{customerId}',function(Request $request,Response $response,array $parms){	
	...
	$app->delete('/deleteuser/{customerId}',function(Request $request,Response $response,array $parms){	
		$customerId = $parms['customerId'];	
		$args = json_decode($request->getBody(),true);		
		$modifiedDate     = date("Y-m-d H:i:s");
		$modifiedBy 	 = $args["userId"];
